...
function viz2D3D_line_stresses(fem,opts,LC) 
%iterate through the requested load cases
for iLC = 1:size(LC,2)  
    %Output Load Case Number
    fprintf(['Stress plot for "', opts.nameProblem, '"\n']);
    ...
    fprintf(['Max stress: %3.', num2str(rd) ,'f MPa \n'],max_axis);
    min_axis = round(min(eS),rd);
    fprintf(['Min stress: %3.', num2str(rd) ,'f MPa \n'],min_axis);
    ...
    switch opts.slv.elemType
        case 'truss'
            title(['Normal Stress, LC ', num2str(LC(iLC))]);
        case 'beam'
            title(['von Mises Stress, LC ', num2str(LC(iLC))]);
    end
